...
[https://www.codewars.com/kata/51eead3461ccf7db04000017/train/javascript](https://www.codewars.com/kata/51eead3461ccf7db04000017/train/javascript)
...
For extra style points you can have your solution handle array values as query parameters, however there are no tests that explicitly test for them.
...
class UriBuilder {
    constructor(uri) {
        this.params = {};
        this.domain = '';
        const [_domain, _query] = uri.split('?');
        if (_domain) {
            this.domain = _domain;
        }
        if (_query) {
            this.params = this.parseParams(_query);
        }
    }
    ...
    parseParams(query = '') {
        const queryKVs = query.split('&');
        // this kata's test env is too old, so it is not allowed to use ES6 method like `Object.fromEntries()`
        let paramsObj = {};
        queryKVs.forEach((kv) => {
    ...
    build() {
        return this.domain + this.stringifyParams();
    }
}
```